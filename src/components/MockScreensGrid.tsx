        ...
        <div className="grid w-full gap-4 md:grid-cols-2">
          {project.artifacts.map((artifact) => (
            <div key={artifact.title} className="relative min-h-[420px] overflow-hidden rounded-[1.75rem] bg-[#2d2d2d] p-7">
              ...
              <div className="relative rounded-[1.35rem] border border-white/12 bg-[#f4f0ea] p-5 text-[#1f1f1f] shadow-[0_22px_60px_rgba(0,0,0,0.4)]">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-semibold tracking-tight">{artifact.title}</h3>
                  </div>
                  <span className="rounded-full bg-black px-3 py-1 text-xs font-bold text-white">ROSSKO</span>
                </div>
                <p className="text-sm font-semibold leading-5 text-black/55">{artifact.subtitle}</p>
                {artifact.image ? (
                  <button
                    type="button"
                    onClick={() => setPreviewImage({ title: artifact.title, src: artifact.image! })}
                    ...
                    aria-label={`Открыть превью: ${artifact.title}`}
                  >
                    <img src={artifact.image} alt={artifact.title} className="h-auto w-full object-contain" />
              ...
              <p className="relative mt-5 text-center text-sm font-semibold text-white/45">{artifact.caption}</p>
            </div>
          ))}
        </div>
      </section>
