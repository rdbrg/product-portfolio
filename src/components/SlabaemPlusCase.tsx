...
const realScreens: Record<string, string> = {
  ...
  "Скрин 16.1: предложение Plus при редактировании объясняет, что подписка снимает ограничение 3 днями.":
    "/screens/upsell при редактировании объясняет, что Plus снимает ограничение 3 днями.png",
  ...
  "Скрин 23: ограничение Free на сохраненные поиски показывает базовые возможности радаров.":
  ...
  "Скрин 25: уведомление по радару возвращает пользователя без необходимости вручную проверять сайт.":
  ...
  "Скрин 30: предложение Plus появляется при лимите объявлений и связывает подписку с конкретной задачей.":
...
function RealScreen({ label, src, index, language, onPreview }: { label: string; src: string; index: number; language: Language; onPreview: (screen: PreviewScreen) => void }) {
  ...
  return (
    <article className="relative overflow-hidden rounded-[1.5rem] bg-[#252525] p-5">
      ...
      >
        <Image src={withBasePath(src)} alt={displayLabel} width={1400} height={900} className="h-auto w-full object-cover" />
