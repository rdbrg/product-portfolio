...
export const slabaemPlusCase: SlabaemPlusBlock[] = [
  ...
  },
  {
    type: "screens",
    items: [
      "Скрин 7: состояние лимита, которое показывает ограничение без агрессивного платного барьера.",
  ...
  },
  {
    ...
    items: [
      ...
      "не смешивать Plus с возможностями для бизнес-пользователей;",
  ...
  },
  {
    ...
    items: [
      ...
      "Скрин 10: контекстное предложение Plus в момент реальной потребности.",
    ],
  },
  {
    type: "section",
    ...
    items: [
      ...
      "конверсию в оплату;",
      "выручку подписок.",
      "При этом важно было сохранить здоровый Free-опыт и не увеличить раздражение от монетизации.",
    ],
  },
  {
    type: "section",
    title: "Решение",
    paragraphs: [
      "Я собрал Plus как набор расширенных лимитов для активного пользователя: подписка усиливает базовый Free-режим, но не добавляет возможности для бизнес-пользователей.",
      "В Plus вошли:",
    ],
    items: [
      "снятие ограничения Free на количество опубликованных объявлений;",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Free подходит для базового использования: можно опубликовать до 1 объявления, добавить до 1 фото в объявление, создать до 1 радара и получать подборку по радару раз в неделю. В группах доступны базовые лимиты: до 1 фото, 1 города и 1 жанра. Рекламные блоки и чужие закрепы показываются.",
      "Слабаем Plus снимает ограничение Free на опубликованные объявления, увеличивает лимит фото в объявлении до 10, фото в группе до 20, городов и жанров в группе до 3, а также увеличивает количество радаров до 20. Подборка по радарам приходит ежедневно. Также Plus открывает AI-черновик объявления с лимитом до 3 генераций на одной открытой форме и убирает рекламу и чужие закрепы.",
      "Важно: Plus не дает автоматический топ в выдаче, бесплатный закреп, бейдж доверия или право вести бизнес-раздел. Эти возможности относятся к PRO.",
    ],
  },
  {
    type: "screens",
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "Plus снимает ограничение Free на опубликованные объявления и позволяет вести несколько активных сценариев параллельно.",
    ],
  },
  ...
  { type: "screens", items: ["Скрин 16.1: предложение Plus при редактировании объясняет, что подписка снимает ограничение 3 днями."] },
  {
    type: "section",
    ...
    paragraphs: [
      ...
      "Для групп Plus расширяет лимит до 20 фото, а также позволяет указать до 3 городов и до 3 жанров вместо базового ограничения Free: 1 фото, 1 город и 1 жанр.",
  ...
  { type: "screens", items: ["Скрин 17: счетчик фото во Free-версии заранее показывает доступный лимит.", "Скрин 18: увеличенный лимит Plus помогает подробнее показать проект или инструмент.", "Скрин 19: объявление с большим количеством фото повышает доверие к автору."] },
  {
    ...
    paragraphs: [
      ...
      "Это поддерживает спокойный опыт: активный пользователь, который часто возвращается в продукт, получает более чистый интерфейс без лишнего визуального шума.",
  ...
  { type: "screens", items: ["Скрин 20: Free-интерфейс с рекламным блоком показывает источник визуального шума.", "Скрин 21: Plus-версия убирает рекламу и делает частое использование спокойнее.", "Скрин 22: собственный закреп остается видимым, чтобы пользователь контролировал продвижение."] },
  {
    ...
    paragraphs: [
      ...
      "Это важная часть спокойного опыта: вместо постоянной ручной проверки сайта пользователь получает уведомление, когда появляется что-то подходящее.",
    ],
  },
  { type: "screens", items: ["Скрин 23: ограничение Free на сохраненные поиски показывает базовые возможности радаров.", "Скрин 24: Plus расширяет количество радаров для нескольких параллельных запросов.", "Скрин 25: уведомление по радару возвращает пользователя без необходимости вручную проверять сайт."] },
  {
    type: "section",
  ...
  {
    type: "section",
    title: "Предложение Plus",
    paragraphs: ["Предложение Plus встроено в моменты, где пользователь уже сталкивается с реальной потребностью:"],
  ...
  { type: "screens", items: ["Скрин 30: предложение Plus появляется при лимите объявлений и связывает подписку с конкретной задачей."] },
  {
    type: "section",
    ...
    items: [
      ...
      "из 72 стартов оплаты 46 завершились успешной оплатой, конверсия составила 63,89%;",
  ...
  },
  {
    ...
    items: ["Продуктовые:", "количество созданных объявлений;", "количество опубликованных объявлений;", "редактирования объявлений;", "редактирования позже 3 дней после публикации;", "количество откликов;", "использование фото;", "создание сохраненных поисков;", "открытия и клики из уведомлений по радарам.", "Монетизация:", "конверсия из старта оплаты в успешную оплату;", "доля пользователей с активным Plus;", "выручка Plus;", "средний чек;", "продления подписки;", "отток пользователей.", "Качество:", "жалобы на рекламу;", "отключения уведомлений;", "доля пользователей, упирающихся в лимиты;", "доля пользователей, которые начинают оплату после предложения Plus, но не завершают ее."],
  },
  ...
  },
  {
    ...
    title: "Вывод",
    paragraphs: [
      "Слабаем Plus — это не платный барьер ради монетизации, а продуктовый слой для активных пользователей.",
...
export const slabaemPlusCaseEn: SlabaemPlusBlock[] = [
  ...
  },
  {
    ...
    title: "Solution",
    paragraphs: [
      "I assembled Plus as a set of expanded limits for active users: the subscription expands Free limits but does not include PRO business capabilities.",
      "Plus included:",
    ],
    items: [
